  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const others = allData.blogs.filter(
          (b: BlogPost) => b.blog_id !== Number(id)
        );
            ...
            <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed">
              {blog.blog_content.split("\n").map((para, i) =>
                para.trim() ? (
                  <p
                    key={i}
                    className="mb-5 text-base md:text-lg leading-relaxed text-gray-600"
                  >
                    {para}
                  </p>
                ) : (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h4 className="text-lg font-bold text-gray-900">
                    {blog.author_name || "Gemellery Team"}
                  </h4>
                  <p className="text-sm text-gray-500">
                    {blog.author_email || "team@gemellery.com"}
                  </p>
                </div>
              </div>
            </div>
