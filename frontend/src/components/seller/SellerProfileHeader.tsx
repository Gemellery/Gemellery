      ...
      <div className="px-5 sm:px-8 pb-6 -mt-14 relative z-10">
        ...
        <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: "Member Since", value: memberYear.toString() },
            { label: "Total Sales", value: seller.totalSales.toString() },
            { label: "Response Rate", value: `${seller.responseRate}%` },
          ].map(({ label, value }) => (
            <div
              key={label}
              className="bg-background rounded-[var(--radius-lg)] px-4 py-3 border border-border"
            >
